...
class webController extends Controller {
    ...
    protected function lists ($model,$where=array(),$order='',$rows=0,$base = array('status'=>array('egt',0)),$field=true, $group=''){
        $options    =   array();
		...
		if(empty($group)){
			...
			$total        =   $model->where($options['where'])->group($group)->select();
			$total = count($total);
		}
	...
	/**
	 * 生成静态文件
	 * @param  unknow   $content   内容
	 * @param  unknow   $filePath  地址
	 * @param  int      $html      模板
	 */
	public  function   getHtml($content, $filePath,  $html = 'index')
	{
        $filePath = $filePath;
		...
		$contents = '';
		/*
		preg_match_all('/<img.*?src="(.*?)".*?>/is', $contents ,$match); //获取图片
		//写入缓存
		$filename="Html/Html/Manifest/index.manifest";
		...
		fclose($handle);
		*/
		$contents = $this->fetch('Admin@Public:index');
	    $fp = fopen($filePath,"w");		  
	    fwrite($fp, $contents);
	    fclose($fp);
	}
